...
def get_explore_products(limit: int = 50) -> List[Dict[str, Any]]:
    supabase = get_supabase_client()
    try:
        select_cols = (
            "product_id, product_name, product_img, start_price, end_time, status, views, likes, "
            "auction_id, listed_by, category, description, condition, current_bid, total_bids, source"
        )
        resp = (
            supabase
            .from_("product")
